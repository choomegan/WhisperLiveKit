...
class AudioProcessor:
    ...
    async def get_current_state(self):
        ...
        async with self.lock:
            ...
            if self.tokens:
                latest_end = max(
                    self.end_buffer, self.tokens[-1].end if self.tokens else 0
                )
                remaining_diarization = max(
                    0, round(latest_end - self.end_attributed_speaker, 2)
                )
    ...
    async def ffmpeg_stdout_reader(self):
        ...
        while True:
            try:
                ...
                if len(self.pcm_buffer) >= self.bytes_per_sec:
                    ...
                    # Process audio chunk
                    pcm_array = self.convert_pcm_to_float(
                        self.pcm_buffer[: self.max_bytes_per_sec]
                    )
        ...
        logger.info(
            "FFmpeg stdout processing finished. Signaling downstream processors."
        )
        if self.args.transcription and self.transcription_queue:
        ...
        while True:
            try:
                ...
                if not self.online:  # Should not happen if queue is used
                    logger.warning(
                        "Transcription processor: self.online not initialized."
                    )
                    self.transcription_queue.task_done()
                    continue

                asr_internal_buffer_duration_s = (
                    len(getattr(self.online, "audio_buffer", []))
                    / self.online.SAMPLING_RATE
                )
                ...
                duration_this_chunk = (
                    ...
                    if isinstance(pcm_array, np.ndarray)
                    else 0
                )
    ...
    async def diarization_processor_chunked(self, diarization_obj):
        """
        Offline diarization of accumulated audio for longer context. Results in delayed
        (but hopefully more accurate) diarization results.
        """
        chunk_len = diarization_obj.chunk_len
        overlap = diarization_obj.overlap

        chunk_samples = chunk_len * self.sample_rate
        stride = chunk_samples - overlap * self.sample_rate

        total_audio = deque()
        idx = 0  # chunk index
        t0 = 0.0  # time offset for current chunk (seconds)

        # collect audio until 90 seconds
        while True:
            pcm_array = await self.diarization_queue.get()
            if pcm_array is SENTINEL:
                logger.debug("Diarization processor received sentinel.")
                self.diarization_queue.task_done()
                break
            total_audio.extend(pcm_array.tolist())  # pcm array here is a numpy arr

            # once audio > 90 seconds, send that chunk to the diarizer
            while len(total_audio) >= chunk_samples:
                chunk = np.fromiter(
                    itertools.islice(total_audio, 0, chunk_samples), dtype=np.float32
                )  # use itertools to prevent copy deque

                # remove processed chunk from the buffer
                for _ in range(int(stride)):
                    total_audio.popleft()

                results = diarization_obj.diarize_chunk(
                    wav_chunk=chunk,
                    t0=t0,
                    sr=self.sample_rate,
                )

                # update diarization object's speaker segments list
                diarization_obj.update_speaker_segments(results)

                # assign speakers to tokens
                async with self.lock:
                    new_end = diarization_obj.assign_speakers_to_tokens(
                        self.end_attributed_speaker,
                        self.tokens,
        ...
        while True:
            try:
                ...
                if (
                    ...
                    and not self.args.transcription
                    and self.args.diarization
                ):
                ...
                if not tokens and not buffer_transcription and not buffer_diarization:
                    ...
                    response_status == "active_transcription"
                    and not final_lines_for_response
                ):
                    final_lines_for_response = [
                        {
                            "speaker": 1,
                ...
                response = {
                    ...
                    "remaining_time_transcription": state[
                        "remaining_time_transcription"
                    ],
                    "remaining_time_diarization": state["remaining_time_diarization"],
                }

                current_response_signature = (
                    f"{response_status} | "
                    + " ".join(
                        [
                            f"{line['speaker']} {line['text']}"
                            for line in final_lines_for_response
                        ]
                    )
                ...
                if self.is_stopping:
                    ...
                    if (
                        self.args.transcription
                        and self.transcription_task
                        and not self.transcription_task.done()
                    ):
                        all_processors_done = False
                    if (
                        self.args.diarization
                        and self.diarization_task
                        and not self.diarization_task.done()
                    ):
                        all_processors_done = False
        ...
        if self.args.diarization and self.diarization:
            if self.args.diarization_type == "online":
                self.diarization_task = asyncio.create_task(
                    self.diarization_processor(self.diarization)
                )
            else:
                self.diarization_task = asyncio.create_task(
                    self.diarization_processor_chunked(self.diarization)
                )
        ...
        while True:
            try:
                ...
                for i, task in enumerate(tasks_to_monitor):
                    if task.done():
                        ...
                        if exc:
                            ...
                            logger.error(
                                f"{task_name} unexpectedly completed with exception:\n{tb}"
                            )
                        else:
                ...
                if ffmpeg_state == FFmpegState.FAILED:
                    logger.error(
                        "FFmpeg is in FAILED state, notifying results formatter"
                    )
